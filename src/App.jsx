...
import { auth, ouvirAuth, cadastrar, entrar, sair, getShared, setShared, getPersonal, setPersonal, ouvirShared } from "./firebase.js";
...
const VANTAGENS = [
  ...
  { nome: "Crítico Aprimorado", desc: "Aumente sua ameaça de crítico com um ataque específico (escolhido quando você adquire esta vantagem) em 1, permitindo que você tire um acerto crítico com um 19 ou 20 naturais. Cada graduação adicional se aplica a um ataque diferente ou aumenta sua ameaça de crítico com um mesmo ataque em mais 1, até um máximo de ameaça de 16-20 com 4 graduações." },
...
function attr(ent, key) { return (ent && ent.atributos && ent.atributos[key]) || 0; }
...
function bonusPericia(ent, nomePericia) {
  ...
  if (!p) return 0;
  return attr(ent, p.atributo) + ((ent.periciaPontos && ent.periciaPontos[nomePericia]) || 0) + bonusVantagemToggle(ent, nomePericia) + calcModCondicoes(ent).testesGerais;
}
...
function deslocamentoTexto(ent) {
  const g = Math.max(-2, Math.min(24, attr(ent, "agilidade")));
  return DESLOC_TABELA[g + 2];
}
function pontosPoderMax(ent) {
  const base = ent?.rotulo === "Invocação" ? 15 : 25;
  return base + (Math.max(1, ent?.nivel || 1) - 1) * 15;
}
function pontosPoderGastos(ent) {
  ...
  const custoHabilidades = (ent?.ataques || []).reduce((s, a) => s + (a.equipamento ? 0 : (a.pp || 0)), 0);
...
function montarTeste(dado, bonus, cd) {
  const total = dado + bonus;
  const diff = total - cd;
  const graus = calcGraus(diff);
  const ehCrit = dado === 20;
...
function RollModal({ contexto, entidades, onFechar, registrar, animar, aplicarDano, atualizarCampo, atualizarCampos }) {
  ...
  const rolarAcerto = () => {
    ...
    const r = montarTeste(dado, bonusFinal, defVal);
    setAtaqueAcertoR(r);
    consumirAjudaSeHouver();
    consumirNossoAlvo();
    consumirBonusTreino();
...
export default function MesaMM3() {
  ...
  /* recarga manual (botão "↻ Atualizar" do histórico) — os dados já chegam sozinhos
     em tempo real pelos listeners abaixo, isso é só um refresh de conveniência */
  const carregarTudo = useCallback(async () => {
    const [ents, hist, ini] = await Promise.all([getShared("entidades"), getShared("historico"), getShared("iniciativa")]);
    setEntidades(Array.isArray(ents) ? ents : []);
  ...
  useEffect(() => {
    ...
    const unsubEnt = ouvirShared("entidades", (v) => setEntidades(Array.isArray(v) ? v : []));
    const unsubHist = ouvirShared("historico", (v) => setHistorico(Array.isArray(v) ? v : []));
  ...
  const salvarEntidades = async (novaLista) => { setEntidades(novaLista); await escreverCompartilhado("entidades", novaLista); };
  const registrar = async (entry) => {
    setHistorico((atual) => {
      const nova = [{ tipo: "sistema", ...entry, id: uid(), hora: horaAgora() }, ...atual].slice(0, 150);
      escreverCompartilhado("historico", nova);
      return nova;
    });
  };
...
function FichaForm({ inicial, rotulos, onSalvar, onCancelar, entidades, atualizarCampo, registrar }) {
  ...
  const addAtaque = () => upd("ataques", [...f.ataques, { id: uid(), nome: "", tipoAcerto: "corpo", custoVida: 0, custoNen: 0, extras: {}, efeitos: [{ id: uid(), ...efeitoPadrao("Dano") }] }]);
